...
public class HpBar extends ProgressBar {
    ...
    public HpBar(Player p) {
        ...
        int height = 20;
        this.setPosition(10, Gdx.graphics.getHeight() - 20);
        getStyle().background = getColoredDrawable(player.hpMax, height, Color.RED);
        getStyle().knob = getColoredDrawable(0, height, Color.GREEN);
        getStyle().knobBefore = getColoredDrawable(player.hpMax, height, Color.GREEN);

        setWidth(player.hpMax);
        setHeight(height);
